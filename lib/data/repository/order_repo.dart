...
class OrderRepo {
  ...
  Future<ApiResponse> placeOrder(
      String addressID, String couponCode, String orderNote) async {
    try {
      final response = await dioClient.get(AppConstants.ORDER_PLACE_URI +
          '?address_id=$addressID&coupon_code=$couponCode&billing_address_id=0&order_note=$orderNote');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> placeOrderEfecty(
      String addressID, String couponCode, String orderNote) async {
    try {
      final response = await dioClient.get(AppConstants.ORDER_PLACE_URI_EFECTY +
          '?address_id=$addressID&coupon_code=$couponCode&billing_address_id=0&order_note=$orderNote');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
  ...
  Future<ApiResponse> placeOrderBaloto(
      String addressID, String couponCode, String orderNote) async {
    try {
      final response = await dioClient.get(AppConstants.ORDER_PLACE_URI_BALOTO +
          '?address_id=$addressID&coupon_code=$couponCode&billing_address_id=0&order_note=$orderNote');
      print('response');
      print(response);
      return ApiResponse.withSuccess(response);
